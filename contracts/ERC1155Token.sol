...
contract ERC1155Token is ERC1155, Ownable {
    ...
    */
    ...
    /*
    add a retailer (retailer_name, token_id associated to retailer)
    */
   function addRetailer(string memory _retailerName, uint256 _tokenid) public {
        names.push(_retailerName);
        ids.push(_tokenid);
        nameToId[_retailerName] = _tokenid;
        idToName[_tokenid] = _retailerName;
   }
    ...
    */
    function mint(address account, uint _id, uint256 amount, string memory _metadata) 
        public payable returns (uint)
    {
        // require(msg.value == mintFee);
        _mint(account, _id, amount, bytes(_metadata));
        return _id;
    }
